...
export default function LabelCarousel() {
  ...
  const fetchLabels = useCallback(async () => {
    ...
    try {
      ...
      if (!labels || !Array.isArray(labels) || labels.length === 0) {
        throw new Error("No Tomato labels found");
      }
  ...
  // Format Tomato name for display
  const formatTomatoName = useCallback((name: string) => {
    return name
      .split(/[_-]/)
    ...
    return (
      <Card className="w-full max-w-md mx-auto">
        <CardContent className="p-6 text-center">
          ...
          <h3 className="text-lg font-semibold mb-2">Failed to Load Tomatos</h3>
    ...
    return (
      <Card className="w-full max-w-md mx-auto">
        <CardContent className="p-6 text-center">
          ...
          <h3 className="text-lg font-semibold mb-2">No Tomatos Available</h3>
          <p className="text-gray-600 mb-4">
            No Tomato data is currently available. Please try again later.
          </p>
          <Button onClick={handleRetry} variant="outline" className="gap-2">
            <RefreshCw className="h-4 w-4" />
            Refresh
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      {/* Header */}
      <CardHeader className="text-center">
        <CardTitle className="text-3xl font-bold text-red-700 flex items-center justify-center gap-2">
          <Leaf className="h-8 w-8" />
          Tomato Gallery
        </CardTitle>
        <p className="text-gray-600 mt-2">
          Explore different Tomatos and their growing seasons
        </p>
      </CardHeader>
                ...
                <CardContent className="p-4 flex flex-col h-full">
                  ...
                  <div className="relative w-full aspect-square rounded-lg overflow-hidden mb-3 bg-gray-100">
                    ...
                    {images[label] && !imageErrors[label] && (
                      <Image
                        src={images[label]}
                        alt={`${formatTomatoName(label)} Tomato`}
                        fill
